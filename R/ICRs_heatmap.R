...
ICRs_heatmap <- function(df_ICR, sampleInfo, control_label = "Control",
    ...
    if (is.null(annotation_col)) {
        ...
        if (!is.list(annotation_col) ||
            ...
            stop(
                "The 'annotation_col' list must have the same number of colors as ",
    ...
    if (plot_type == "beta") {
        ...
        breaks <- c(
            seq(0, 0.5, length.out = ceiling(paletteLength / 2) + 1),
    ...
    # Ordering rows
    row_clust <- if (order_by == "cord") {
        FALSE
    } else if (order_by == "meth") {
        TRUE
    } else {

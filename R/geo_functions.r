    ...
    tryCatch(
        {
            ...
            message(
                "Phenotypic data extracted: ", nrow(pheno_data), " samples, ",
                ncol(pheno_data), " variables"
            )

            # Download supplementary files (IDAT files)
            message("\n=== DOWNLOADING IDAT FILES ===")
            supp_files <- GEOquery::getGEOSuppFiles(geo_id,
...
#' @return NULL (invisibly). Function is called for its side effects of extracting files.
#' @keywords internal
extract_compressed_files <- function(dir_path) {
    ...
    tar_files <- list.files(dir_path,
        pattern = "\\.(tar|tar\\.gz|tgz)$",
        full.names = TRUE, recursive = TRUE
    )
    zip_files <- list.files(dir_path,
        pattern = "\\.zip$",
        full.names = TRUE, recursive = TRUE
    )
    gz_files <- list.files(dir_path,
        pattern = "\\.gz$",
        full.names = TRUE, recursive = TRUE
    )

    message(
        "Found compressed files: ", length(tar_files), " TAR, ",
...
find_and_organize_idats <- function(source_dir, target_dir) {
                ...
                tryCatch(
                    {
                        if (grepl("\\.gz$", compressed_file)) {
                            ...
                            if (requireNamespace("R.utils", quietly = TRUE)) {
                                ...
                                R.utils::bunzip2(compressed_file,
                                    destname = decompressed_file,
                                    overwrite = TRUE, remove = FALSE
                                )
                                idat_files <- c(idat_files, decompressed_file)
    ...
    if (length(red_files) == 0 || length(grn_files) == 0) {
        warning(
            "Missing Red or Green channel files. Found ", length(red_files),
...
create_geo_sample_sheet <- function(pheno_data, idat_files, group_column,
    ...
    if (is.null(group_column)) {
        group_info <- auto_detect_groups(
            pheno_data, sample_matches$pheno_id,
        ...
        sample_groups <- assign_sample_groups(
            pheno_data[sample_matches$pheno_id, group_column],
            control_terms, case_terms
        )
...
auto_detect_groups <- function(pheno_data, sample_ids, control_terms, case_terms) {
            ...
            tryCatch(
                {
                    ...
                    if (length(group_table) == 2 && !all(names(group_table) == "Unknown")) {
                        ...
                        message(
                            "Column '", col, "' score: ", round(total_score, 3),
    ...
    if (is.null(best_column)) {
        ...
        message_text <- paste(
            "Auto-detected groups from column:", best_column,
    ...
    tryCatch(
        {
            ...
            for (i in seq_len(min(5, nrow(summary_info$largest_files)))) {
                cat(
                    "  ", summary_info$largest_files$filename[i],
    ...
    tryCatch(
        {
            ...
            message(
                "Successfully retrieved phenotypic data: ", nrow(pheno_data), " samples, ",
                ncol(pheno_data), " variables"
            )

...
plot_line_ICR <- function(significantDMPs, ICRcpg, ICR, sampleInfo, interactive = TRUE) {
    ...
    if (!ICR %in% significantDMPs$ICR) {
        stop(paste(
            "ICR", ICR, "not found in significantDMPs. Available ICRs:",
    ...
    if (length(sampleInfo) != length(methylation_col_indices)) {
        stop(
            "sampleInfo length (", length(sampleInfo),
    ...
    tryCatch(
        {
            ...
            if (length(unique_labels) != 2) {
                stop(paste(
                    "Expected exactly 2 groups for plotting, but found", length(unique_labels),
            ...
            plot <- ggplot(methylationDataLong, aes(x = cstart, y = Methylation, group = Sample, color = SampleGroup)) +
                ...
                geom_vline(
                    data = data.frame(x = unique(regionDMPs$cstart)),
                ...
                labs(
                    ...
                    subtitle = paste(
                        "Showing", sum(methylationDataLong$IsDMP), "significant DMPs |",

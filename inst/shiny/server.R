...
server <- function(input, output, session) {
  ...
  observeEvent(input$validateGEO, {
    ...
    showModal(modalDialog(
      ...
      div(
        ...
        p("Steps:"),
        tags$ol(
          tags$li("Checking if GEO ID exists"),
          tags$li("Retrieving dataset metadata"),
          tags$li("Checking for methylation data"),
          tags$li("Looking for IDAT files"),
          tags$li("Detecting array platform")
        )
      ),
      footer = NULL,
      size = "m"
    ...
    tryCatch({
      ...
      output$geoValidationResult <- renderPrint({
        cat("GEO Dataset Validation Results\n")
        cat("=============================\n\n")

        if (validation_result$valid) {
          cat("✅ Dataset found and accessible\n\n")
          cat("Title:", validation_result$title, "\n")
          cat("Sample count:", validation_result$sample_count, "\n")

          if (!is.null(validation_result$summary)) {
            cat("Summary:", validation_result$summary, "\n")
          }

          cat("Array platform detected:", validation_result$array_type, "\n")
          cat("Platform IDs:", paste(validation_result$platforms, collapse = ", "), "\n\n")

          if (validation_result$has_idats) {
            cat("✅ IDAT files detected - suitable for GIMP analysis\n")
            cat("✅ This dataset can be processed with GIMP\n")
          } else {
            cat("❌ No IDAT files detected\n")
            cat("❌ This dataset is not suitable for GIMP analysis\n")
            cat("   GIMP requires raw IDAT files for proper analysis\n")
          }

        } else {
          cat("❌ Validation failed\n")
          cat("Error:", validation_result$message, "\n")
        }
      })

      if (validation_result$valid && validation_result$has_idats) {
        showNotification("GEO dataset validated successfully! Ready for processing.",
      ...
      output$geoValidationResult <- renderPrint({
        cat("GEO Validation Error\n")
        cat("===================\n\n")
        cat("Error:", e$message, "\n\n")
        cat("Common issues:\n")
        cat("- GEO ID doesn't exist or is not accessible\n")
        cat("- Network connection problems\n")
        cat("- GEOquery package not properly installed\n")
        cat("- Dataset is private or restricted\n")
      })
    })
  })

  # GEO processing
  observeEvent(input$processGEO, {
    req(values$geo_validation)

    if (!values$geo_validation$valid || !values$geo_validation$has_idats) {
      showNotification("Please validate a suitable GEO dataset first.", type = "warning")
      return()
    }

    # Estimate processing time
    estimated_time <- ceiling(values$geo_validation$sample_count * 0.8)

    showModal(modalDialog(
      title = "Processing GEO Dataset",
      div(
        h4("Downloading and processing GEO data..."),
        p("Dataset:", input$geoId),
        p("Samples:", values$geo_validation$sample_count),
        p("Estimated time:", estimated_time, "minutes"),
        br(),
        div(class = "alert alert-warning",
            icon("clock"),
            strong(" This may take a while! "),
            "Please don't close the browser. Processing includes downloading large files from GEO."
        ),
        br(),
        div(class = "progress progress-striped active",
        ...
        p("Steps:"),
        tags$ol(
          tags$li("Downloading GEO metadata and phenotypic data"),
          tags$li("Downloading supplementary IDAT files"),
          tags$li("Extracting and organizing files"),
          tags$li("Auto-detecting sample groups"),
          tags$li("Creating sample sheet"),
          tags$li("Processing through GIMP pipeline"),
          tags$li("Quality control and normalization"),
          tags$li("Generating final beta matrix")
        )
      ),
      footer = NULL,
      size = "l"
    ...
    tryCatch({
      # Prepare parameters
      group_column <- if (nchar(trimws(input$geoGroupColumn)) > 0) input$geoGroupColumn else NULL
      control_terms <- strsplit(input$geoControlTerms, ",")[[1]]
      control_terms <- trimws(control_terms)
      case_terms <- strsplit(input$geoCaseTerms, ",")[[1]]
      case_terms <- trimws(case_terms)
      max_samples <- input$geoMaxSamples
      n_cores <- if (input$geoEnableParallel) input$geoNCores else NULL

      # Process GEO dataset
      geo_results <- process_geo_dataset(
        geo_id = input$geoId,
      ...
      # Store results
      values$betaMatrix <- geo_results$beta_matrix
      values$sample_sheet <- geo_results$sample_sheet
      values$qc_metrics <- geo_results$qc_metrics
      values$geo_metadata <- geo_results$geo_metadata

      # Auto-assign sample groups from GEO processing
      if (!is.null(geo_results$sample_info)) {
        values$sampleInfo <- geo_results$sample_info

        # Auto-process if groups are available and not all "Unknown"
        if (!all(values$sampleInfo == "Unknown")) {
          # Map GEO array type to GIMP bedmeth
          ...
          # Create UI for manual group assignment if auto-detection failed
          output$sampleGroupUI <- renderUI({
            samples <- colnames(values$betaMatrix)
            ...
            tagList(
              div(class = "alert alert-warning",
                  icon("exclamation-triangle"),
                  " Could not auto-detect sample groups from GEO data. Please assign groups manually."
              ),
              selectInput("controlSamples", "Select Control Samples:",
                          choices = samples,
                          multiple = TRUE,
              ...
              helpText("Samples not selected as controls will be assigned to the Case group."),
              actionButton("processDataGEO", "Process with Group Assignment",
                          class = "btn-primary", icon = icon("play"))
            )
          })
        }
      ...
      success_msg <- paste("GEO dataset processed successfully!",
                          ncol(values$betaMatrix), "samples,",
                          nrow(values$betaMatrix), "probes from", input$geoId)
      showNotification(success_msg, type = "message", duration = 10)

      # Generate QC outputs and GEO metadata
      generateQCOutputs()
      generateGEOMetadata()

    }, error = function(e) {
      removeModal()
      error_msg <- paste("GEO processing failed:", e$message)
      showNotification(error_msg, type = "error", duration = 20)

      # Detailed error logging
      cat("=== GEO PROCESSING ERROR ===\n")
      cat("GEO ID:", input$geoId, "\n")
      cat("Error:", e$message, "\n")
      cat("============================\n")
      print(traceback())
    })
  ...
  # Handle manual group assignment for GEO data
  observeEvent(input$processDataGEO, {
    req(values$betaMatrix, input$controlSamples, values$geo_metadata)

    showModal(modalDialog(
      title = "Processing GEO Data for GIMP Analysis",
      "Extracting ICRs and creating analysis matrices...",
      footer = NULL
    ))

    # Create sample info for GEO data
    samples <- colnames(values$betaMatrix)
    values$sampleInfo <- ifelse(samples %in% input$controlSamples, "Control", "Case")
    ...
    tryCatch({
      ...
      success_msg <- paste("GEO data processed for GIMP analysis!",
                          "Groups:", paste(table(values$sampleInfo), collapse = " vs "))
      showNotification(success_msg, type = "message")
